...
!          A=fslla*photosynthesis_sl+(one_dp-fslla)*photosynthesis_sh ! A=photosysnthesis (umol co2/m2/s)
    ...
    End If
    If(debug) Then
       Write(33,12) Nint(simtime%time),parlsh,parlpp,plai,clustf,rkbl,sinb,asssh,asssl,fslla,&
            temp,patm_pa,Units_L(4)/Units_L(Unit_L_Input),fact,&
            alphaAvg,frdf,dsinb,apar,pardf,pardr,atmtr,detr,ClimateData%global_rad,detr*dsinb,sinb,dsinbe
    Else
       Write(33,12) Nint(simtime%time),null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,&
            null_dp,null_dp,null_dp,&
            null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp,null_dp
    End If
    If(Farquhar_used) Then
...
2   Format(A)
  ...
  Subroutine WritePlantOutput(SimTime)
    ...
    if(firstCall) Then
       ...
       Open(33,file='original_Photosynthese.out',Status='REPLACE',ACTION='WRITE',&
               FORM='FORMATTED',iostat=ios)
       If(ios/=0) Call WriteError( 'Error while opening plants output file: original_Photosynthese.out')

       Write(33,25) 'time','parlsh','parlpp','plai','clustf','rkbl','sinb','asssh','asssl','fslla',&
            'temp','patm_pa','Units_L(4)/Units_L(Unit_L_Input)','fact',&
            'alphaAvg','frdf','dsinb','apar','pardf','pardr','atmtr','detr','ClimateData%global_rad','detr*dsinb','sinb','dsinbe'     
       If(farquhar_used) Then
...
!               '(-)','(1/s)','(1/s)','(1/s)','(umol photons/m2/s)','(W/m2/sr/um)'
          ...
          Write(64,37) 'time','VCmax','fq%w_c','fq%w_j_sh','fq%w_j_sl','fq%w_e',&
